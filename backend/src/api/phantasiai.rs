...
fn phantasiai_store_failed(
    context: &'static str,
    error: impl std::fmt::Display,
) -> axum::response::Response {
    tracing::error!(%error, context, "phantasiai store failed");
    (
...
async fn get_annotations(
    ...
    let existing = phantasi_annotations::Entity::find()
    ...
    let item = match phantasi_items::Entity::find_by_id(item_id).one(&db).await {
        ...
        Err(e) => {
            return phantasiai_store_failed("load article", e);
        }
    };
    ...
    match ai_parse_with_retry(
        || {
            crate::services::ai_cost_ledger::with_site_ai_ledger(
                ...
                "phantasiai",
                "annotate",
                ai_analyzer.analyze_json("", &prompt, "phantasi_annotations", None),
            )
        },
        parse_annotations,
    )
    .await
    {
        Ok((annotations, detected_language)) => {
            ...
            for ann in &annotations {
                ...
                let active = phantasi_annotations::ActiveModel {
                    item_id: Set(item_id),
        ...
        Err(PhantasiaiAiFailure::Unusable(e)) => {
        ...
        Err(PhantasiaiAiFailure::Provider(e)) => {
...
const PHANTASIAI_AI_ATTEMPTS: u8 = 2;

/// 模型没写好 vs 供应商挂了。两者在界面上不是一回事：前者再抽一次可能就好，
/// 后者是站长要去配的。
enum PhantasiaiAiFailure {
    Provider(String),
    Unusable(String),
}

/// 一次「调模型 → 严格解析」，模型这一把没写好就再抽一次（[`PHANTASIAI_AI_ATTEMPTS`]）。
/// 供应商失败（没配 key、网关挂了）立刻上抛，不重试。
async fn ai_parse_with_retry<T, F, Fut>(
    mut call: F,
    parse: impl Fn(&str) -> Result<T, String>,
) -> Result<T, PhantasiaiAiFailure>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<Output = anyhow::Result<String>>,
{
    let mut last = String::from("empty response");
    for attempt in 0..PHANTASIAI_AI_ATTEMPTS {
        let raw = call()
            .await
            .map_err(|error| PhantasiaiAiFailure::Provider(error.to_string()))?;
        match parse(&raw) {
            ...
            Err(reason) => {
                tracing::warn!(
                    ...
                    "Phantasiai AI response was unusable"
                );
                last = reason;
            }
        }
    }
    Err(PhantasiaiAiFailure::Unusable(last))
}
...
use crate::models::entities::phantasi_podcasts;
...
async fn podcast_script(
        ...
        .await
    {
        ...
        let response = phantasi_podcasts::PodcastResponse::from(cached);
    ...
    match ai_parse_with_retry(
        || {
            crate::services::ai_cost_ledger::with_site_ai_ledger(
                ...
                "phantasiai",
                "podcast",
                ai_analyzer.analyze_json("", &prompt, "phantasi_podcast", None),
            )
        },
    ...
    .await
    {
        Ok((dialogues, language)) => {
            ...
            // 保存到数据库
            let podcast_model = phantasi_podcasts::ActiveModel {
                item_id: Set(item_id),
...
async fn regenerate_podcast_script(
    ...
    // 删除现有播客脚本缓存
    match phantasi_podcasts::Entity::delete_many()
        .filter(phantasi_podcasts::Column::ItemId.eq(item_id))
        .exec(&db)
        .await
    {
    ...
    // 清理 TTS 音频缓存目录
    // 结构: `{phantasi}/{source_id}/{item_id}/tts/`
    let tts_dir = paths()
        .phantasi
...
async fn generate_style_tags(
        ...
        .await
    {
        ...
        Err(e) => {
            return phantasiai_store_failed("load source", e);
        }
    };
        ...
        .await
    {
        Ok(items) => items,
        Err(e) => {
            return phantasiai_store_failed("load articles", e);
        }
    };
...
mod ai_retry_tests {
    ...
    fn every_reader_facing_call_retries_before_failing() {
        let source = include_str!("phantasiai.rs");
        ...
        assert!(super::PHANTASIAI_AI_ATTEMPTS > 1, "次数是 1 等于没有重试");
    }
    ...
    fn a_broken_provider_is_not_retried() {
        ...
        let shell = source
            ...
            .expect("retry shell");
        // 调用失败直接 `?` 上抛，不进循环的下一轮。
        assert!(shell.contains("PhantasiaiAiFailure::Provider(error.to_string()))?"));
        assert!(shell.contains("PhantasiaiAiFailure::Unusable(last)"));
    }
    ...
    fn only_the_object_rooted_calls_ask_for_json_mode() {
        let source = include_str!("phantasiai.rs");
        assert!(source.contains(r#"analyze_json("", &prompt, "phantasi_annotations", None)"#));
        assert!(source.contains(r#"analyze_json("", &prompt, "phantasi_podcast", None)"#));
