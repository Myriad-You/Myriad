...
//! on the notification (this module) or confirms `seo.apply` in chat.
...
pub async fn run_scheduled_seo_review(db: &DatabaseConnection) -> Result<SeoReviewOutcome, String> {
    let inspect = crate::api::seo::public_geo_inspect(db).await;
...
async fn judge_with_ai(
    ...
    let language = crate::api::seo_geo::resolve_language("", title, description);
