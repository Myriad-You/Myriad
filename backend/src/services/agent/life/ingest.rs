...
const LIFE_OWNED_NOTIFY: &[&str] = &["agent.life.platform_activity"];

pub async fn life_enabled() -> bool {
...
/// Existing producers keep notifying unless life is on and the addressee is mid-conversation.
...
pub fn spawn_diary(user_id: i32, summary: impl Into<String>) {
    ...
    tokio::spawn(async move {
        ...
        if let Err(error) = insert_diary(&db, user_id, &summary, "event").await {
            tracing::debug!(%error, user_id, "[Agent life] diary write failed");
        }
    });
}
...
pub fn spawn_presence(user_id: i32) {
    ...
    tokio::spawn(async move {
        ...
        crate::services::agent::life::maybe_apply_departure(&db, user_id).await;
        ...
        if recently_spoke_event(&db, user_id, "agent.life.greeting", 12 * 60)
            .await
            .unwrap_or(true)
        {
            return;
        ...
        if let Err(error) = ingest(&db, user_id, "agent.life.greeting", summary).await {
            tracing::debug!(%error, user_id, "[Agent life] presence ingest failed");
        }
    });
}
...
async fn apply_task_mood(db: &DatabaseConnection, user_id: i32, event_key: &str, mood: f64) {
    ...
    if !is_extremely_low(mood) && is_extremely_low(next) {
        spawn(user_id, "agent.life.mood_floor", "跟这个人的心情掉到了极低");
    }
}
...
最近对这个人说过：\n{recent_block}",
    ...
    match analyzer.analyze_with_system(&system, &prompt).await {
        ...
        Err(error) => {
            tracing::debug!(%error, "[Agent life] Lite speech failed, using fallback");
            fallback
        }
    }
}
