...
pub fn register(registry: &mut CapabilityRegistry) {
    // Phantasi 内容读取（读 DB phantasi_items + phantasi_sources）
    registry.register(Capability {
        id: "phantasi.read".to_string(),
        name: "Read feeds".to_string(),
        description: "Read Phantasi articles from the database.".to_string(),
    ...
    // 订阅源列表（读 DB，支持宽松名称匹配）
    registry.register(Capability {
        id: "phantasi.sources".to_string(),
        name: "Feed list".to_string(),
        description: "List or find Phantasi feeds.".to_string(),
        category: CapabilityCategory::DataRead,
        ...
        input_schema: json!({
            ...
            "properties": {
                ...
                "sourceType": { "type": "string", "description": "Source type: rss|link|phantasiai; friendlink/友链/友情链接 count as link" },
    ...
    registry.register(Capability {
        ...
        input_schema: json!({
            "type": "object",
            "properties": {
                "sourceId": { "type": "integer", "description": "Feed id (prefer the id returned by phantasi.sources)" },
    ...
    // 阅读统计（聚合 phantasi_sources / phantasi_items / phantasi_user_states）
    registry.register(Capability {
        id: "phantasi.stats".to_string(),
        name: "Reading stats".to_string(),
        description: "Show Phantasi reading stats.".to_string(),
        category: CapabilityCategory::DataRead,
    ...
    // 文章内容（读 DB phantasi_items）
    registry.register(Capability {
        id: "phantasi.article".to_string(),
        name: "Article".to_string(),
        description: "Load one Phantasi article by id, guid, or URL.".to_string(),
        category: CapabilityCategory::DataRead,
    ...
    // 订阅源发现
    registry.register(Capability {
        id: "phantasi.discover".to_string(),
    ...
    // 添加订阅
    registry.register(Capability {
        id: "phantasi.subscribe".to_string(),
    ...
    // 生成阅读列表（默认仅本地 phantasi_items；联网需 allowWebSearch=true）
    registry.register(Capability {
        id: "phantasi.generateReadingList".to_string(),
    ...
    // Phantasi 页面内容
    registry.register(Capability {
        id: "phantasi.page".to_string(),
        name: "Phantasi page".to_string(),
        description: "Read Phantasi page content, including feed lists.".to_string(),
        category: CapabilityCategory::DataRead,
        supported_actions: vec![IntentAction::Query],
        input_schema: json!({
            "type": "object",
    ...
    // Phantasi 调度控制
    registry.register(Capability {
        id: "phantasi.schedule".to_string(),
        name: "Phantasi schedule".to_string(),
        description: "Start, stop, or refresh the Phantasi scheduler.".to_string(),
        ...
        required_permissions: vec!["phantasi:admin".to_string()],
        requires_ai: false,
        estimated_duration_ms: Some(30000),
        requires_confirmation: true,
        confirmation_message: Some(
            "This will start, stop, or refresh the Phantasi scheduler".to_string(),
        ),
        risk_level: RiskLevel::Medium,
        ..Default::default()
    });
...
mod tests {
    ...
    #[test]
    fn add_subscription_source_requires_phantasi_manage() {
        // phantasi.subscribe → POST /api/phantasi/sources：privileged phantasi:manage，不是 phantasi:write。
