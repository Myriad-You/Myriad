...
pub(crate) fn tapp_permission_replacement_hint(permission: &str) -> Option<&'static str> {
    match permission {
        ...
        "media:control" => Some(
            "use 'media:playback', 'media:volume' or 'media:queue' instead; update the TAPP Manifest, then update or reinstall the app",
        ),
        _ => None,
    }
}
...
mod tests {
    ...
    fn media_permissions_split_three_basic_action_domains() {
        ...
        let config = DynamicConfig::default();
        assert_eq!(
            TappPermission::MediaPlayback.level(),
            PermissionLevel::Basic
        );
    ...
    #[test]
    fn retired_media_control_error_recommends_action_permissions_but_stays_fail_closed() {
        assert!(TappPermission::from_str("media:control").is_none());
        ...
        .unwrap_err();
        let message = error.message();
        assert!(message.contains("media:playback"), "{message}");
        assert!(message.contains("media:volume"), "{message}");
        assert!(message.contains("media:queue"), "{message}");
        assert!(message.contains("reinstall"), "{message}");
    }
}
