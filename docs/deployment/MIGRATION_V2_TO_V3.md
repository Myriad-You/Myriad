...
| [examples/v3.env.example](./examples/v3.env.example) | 按 kiseki.blog 运维形状脱敏的 `.env` 示例（commit / preview / `HTTP_PORT=8080`） |
...
3. **编辑 `.env`（保留旧 secret，只加缺口）**：
   ...
   示例中的 `MYRIAD_TAG=dev-f80caf1`、`CHANNEL=preview`、`UPDATE_MODE=commit`、
   `HTTP_PORT=8080`、`https://kiseki.blog` 仅为运维形状参考，**请改成你的实际值**。
