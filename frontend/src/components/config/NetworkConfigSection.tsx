  ...
  return (
    ...
    >
      ...
      <InfoCard
        ...
        content={
          <>
            {t.config.networkProxyInfo ||
              '如果您的服务器位于中国大陆，可能需要配置代理才能正常访问 GitHub OAuth、Gemini AI 等外部服务。您可以选择以下方式：'}
            <br />
            <br />
            <strong>
              1. HTTP/SOCKS
              {t.config.proxyOption || '代理'}
            </strong>
            ：
            {t.config.proxyOptionDesc ||
              '配置代理服务器地址，所有外部请求将通过代理发送。'}
            <br />
            <strong>
              2. API
              {t.config.mirrorOption || '镜像服务'}
            </strong>
            ：
            {t.config.mirrorOptionDesc ||
              '使用第三方 API 镜像/中转服务，无需配置代理。'}
          </>
        }
      ...
      <SettingGroup
        ...
        icon={<FaExchangeAlt />}
        description={
          t.config.apiMirrorConfigHint ||
          '使用第三方 API 镜像服务，可替代代理配置'
        }
      >
        <InputItem
