...
interface PhantasiSourceTileProps {
  ...
  layoutOverride?: PhantasiTileLayout
...
function siteHost(source: PhantasiSource): string {
  try {
...
export const PhantasiSourceTile = memo(
  ({
    source,
    size,
    role,
    ...
    const layout: PhantasiTileLayout =
    ...
    const openItem = (item: PhantasiItemPreview) => onOpenItem?.(item, source)
    /** 头条可点语义：role=link，Enter / 空格触发。 */
    const leadLinkProps = (item: PhantasiItemPreview) =>
      onOpenItem
        ? {
            role: 'link' as const,
            tabIndex: 0,
            ...
            >
              ...
              >
                ...
                <ExternalLink
                  ...
                  aria-label={t.phantasi.tileOpenSite}
                />
              </div>
              {desc ? (
                <span
    ...
    if (layout === 'cadence') {
      ...
      const axis = (
        ...
        >
          <span>
            {format(t.phantasi.tileQuietMonths, { months })}
          </span>
          <span
            className={alert ? 'text-red-500 dark:text-red-400' : ''}
          ...
          >
            {alert
              ? format(t.phantasi.tileFailedTimes, { count: source.error_count })
              : t.phantasi.tileToday}
          </span>
        </TileMeta>
      )
        ...
        return (
          ...
          >
            ...
            <div className="relative min-h-0 flex-1 overflow-hidden">
              <div
                className={rotating ? 'phantasi-rotate-track' : undefined}
                style={
                  rotating
                    ? {
                        height: `${pages.length * 100}%`,
                        animationName: `phantasi-rotate-${pages.length}`,
