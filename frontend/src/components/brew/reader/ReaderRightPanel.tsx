        ...
        >
          <div className={`flex flex-col items-center gap-2 p-2 rounded-2xl backdrop-blur-md border ${currentTheme.border} ${currentTheme.surface}`}>
            ...
            <button
              ...
              title={(t.brew as Record<string, string>)[currentFont.labelKey] || currentFont.labelKey}
            ...
            <button
              onClick={cycleLayout}
              className={sideButtonClass}
              title={(t.brew as Record<string, string>)[currentLayout.labelKey] || currentLayout.labelKey}
            >
            ...
            {/* 字号调整 */}
            <div className={`flex flex-col items-center gap-1 p-1 rounded-xl ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
              <button
              ...
              <span className={`text-[10px] ${currentTheme.secondary} tabular-nums`}>{fontSize}</span>
              <button
                onClick={() => adjustFontSize(-1)}
                className={`p-1.5 rounded-lg ${currentTheme.secondary} hover:${currentTheme.text} transition-colors`}
            ...
            <div className={`flex flex-col items-center gap-1 p-1 rounded-xl ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
              ...
              <span className={`text-[10px] ${currentTheme.secondary} tabular-nums`}>{lineHeight.toFixed(1)}</span>
              <button
                onClick={() => adjustLineHeight(-0.1)}
                className={`p-1.5 rounded-lg ${currentTheme.secondary} hover:${currentTheme.text} transition-colors`}
