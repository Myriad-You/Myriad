 ...
 */
function findCompleteCacheVoices(cache: ArticleCacheResponse, dialogueCount: number): { hostVoiceId: number, guestVoiceId: number, voiceName: string } | null {
  if (!cache.voices.length || dialogueCount <= 0)
    return null
...
export interface UsePodcastReturn {
  ...
  // Refs
  podcastListRef: React.RefObject<HTMLDivElement>
}
...
export function usePodcast({
  ...
  const [cloudTtsAvailable, setCloudTtsAvailable] = useState<boolean | null>(null)
        ...
        if (cloudTtsAvailable === null) {
          ...
          try {
            ...
            if (!status.available || !status.tts_enabled) {
              ...
              showToastMessage(status.error || t.brew.cloudTtsUnavailableCheck, 5000)
              setTtsEngine('system')
          ...
          showToastMessage(cloudTtsError || t.brew.cloudTtsUnavailableCheck, 5000)
          setTtsEngine('system')
          saveTTSSettings({ engine: 'system' })
          return
        }
        ...
        if (cloudPodcastPlayerRef.current?.hasAudio()) {
          ...
          try {
            ...
            if (completeCache) {
              ...
              showToastMessage(`${t.brew.switchedToCloudTts}（${t.brew.cached}: ${completeCache.voiceName}）`)
            }
            else {
              // 无完整缓存，回退
              setTtsEngine('system')
              ...
              showToastMessage(cache.voices.length > 0 ? t.brew.cloudCacheIncomplete : t.brew.noCloudCache)
            }
          }
          catch (err) {
  ...
  }, [podcastState, ttsEngine, podcastDialogues, podcastLanguage, cloudTtsAvailable, cloudTtsError, sourceId, itemId, showToastMessage, t])
    ...
    if (podcastDialogues.length > 0 && cloudPodcastPlayerRef.current) {
      ...
      try {
        ...
        const result = await cloudPodcastPlayerRef.current.load(
          podcastDialogues,
          { sourceId, articleId: itemId, hostVoiceId: newHostVoiceId, guestVoiceId: newGuestVoiceId },
        )
  ...
  }, [podcastDialogues, sourceId, itemId, hostVoiceId, guestVoiceId, articleCache, showToastMessage, t])

  // 清除特定音色缓存
  const handleClearVoiceCache = useCallback(async (voiceId: number) => {
    if (clearingVoiceId !== null)
      return
  ...
  }, [sourceId, itemId, clearingVoiceId, articleCache, loadArticleCache, showToastMessage, t])

  // 加载播客脚本
  const loadPodcast = useCallback(async () => {
    if (!isBrewlia || podcastLoading || cloudTtsLoading)
      return
        ...
        if (cloudTtsAvailable) {
          try {
            ...
            if (completeCache) {
              ...
              const result = await cloudPodcastPlayerRef.current?.load(response.dialogues, {
                sourceId,
                articleId: itemId,
              ...
              showToastMessage(cache.voices.length > 0 ? t.brew.cloudCacheIncomplete : `${response.dialogues.length}`)
            }
          }
          catch (cacheErr) {
  ...
  }, [isBrewlia, podcastLoading, cloudTtsLoading, itemId, sourceId, cloudTtsAvailable, showToastMessage, t])
  ...
  const reloadCloudTTS = useCallback(async () => {
    ...
    try {
      const result = await cloudPodcastPlayerRef.current?.load(podcastDialogues, {
        sourceId,
        articleId: itemId,
        hostVoiceId,
        guestVoiceId,
  ...
  }, [podcastDialogues, cloudTtsAvailable, cloudTtsLoading, sourceId, itemId, hostVoiceId, guestVoiceId, showToastMessage, t])
  ...
  const handlePodcastNext = useCallback(async () => {
    if (podcastCurrentIndex >= podcastDialogues.length - 1)
      return
    const newIndex = podcastCurrentIndex + 1
    const player = getActivePlayer()
    if (player) {
  ...
  // 自动滚动到当前播放的对话
  useEffect(() => {
    if (!showPodcastPlayer || podcastDialogues.length === 0)
      return
