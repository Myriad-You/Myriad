...
// 获取来源类型标签和提示
function getSourceTypeInfo(sourceType: SourceType, t: TranslationKeys): { label: string, tooltip: string, isBrewlia: boolean } {
  switch (sourceType) {
    case 'brewlia':
      return {
        label: 'Brewlia AI',
...
function formatTime(timestamp: number | null, t: TranslationKeys): string {
  ...
  if (diff < 60000)
    return t.brew.justNow
  if (diff < 3600000)
    return t.brew.minutesAgo.replace('{minutes}', String(Math.floor(diff / 60000)))
  if (diff < 86400000)
    return t.brew.hoursAgo.replace('{hours}', String(Math.floor(diff / 3600000)))
  if (diff < 604800000)
    return t.brew.daysAgo.replace('{days}', String(Math.floor(diff / 86400000)))
  return date.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })
}

export const SourceCard = React.memo(forwardRef<HTMLDivElement, SourceCardProps>(({
  source,
  index,
  onSourceClick,
  onRefreshSource,
  ...
  const handleIconLoad = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    if (source.theme_color || !source.icon)
      return
  ...
  const hoverClasses = useMemo(() => {
    if (!enableHover || isDragging || isDragOver)
      return ''
                    ...
                    ? (
                        <>
                          ...
                          <span className="text-gray-400 dark:text-gray-500">
                            /
                            {t.brew.articlesCount.replace('{count}', String(source.item_count))}
                          </span>
                        </>
                      )
                    : (
