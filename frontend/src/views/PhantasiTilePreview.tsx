...
import type { PhantasiTileLayout, PhantasiTileSize } from '../components/phantasi/logic/layout'
import type { PhantasiViewerRole } from '../components/phantasi/logic/score'
import type { PhantasiTopic } from '../components/phantasi/logic/topics'
import type { PhantasiItemPreview, PhantasiSource } from '../types/phantasi'

import { useMemo, useState } from 'react'

import { allowedTileSizes } from '../components/phantasi/logic/layout'
import { clusterTopics } from '../components/phantasi/logic/topics'
import { PhantasiFeaturedTile } from '../components/phantasi/tiles/PhantasiFeaturedTile'
import { PhantasiSourceTile } from '../components/phantasi/tiles/PhantasiSourceTile'
import { PhantasiTopicTile } from '../components/phantasi/tiles/PhantasiTopicTile'
...
const CELL = 82
const SIZE_PX: Record<PhantasiTileSize, { width: number; height: number }> = {
...
let seq = 0
function preview(over: Partial<PhantasiItemPreview> = {}): PhantasiItemPreview {
  seq += 1
  return {
    id: 9000 + seq,
...
function source(over: Partial<PhantasiSource> = {}): PhantasiSource {
  return {
    id: 1,
    user_id: 1,
    name: '示例订阅源',
...
interface Case {
  label: string
  layout?: PhantasiTileLayout
  src: PhantasiSource
  items?: PhantasiItemPreview[]
}
...
function makeTopic(withCovers: boolean): PhantasiTopic {
...
const ROLES: PhantasiViewerRole[] = ['guest', 'member', 'admin']

export default function PhantasiTilePreview() {
  const [role, setRole] = useState<PhantasiViewerRole>('member')
  ...
  return (
    <div className="min-h-screen px-6 py-10">
      <div className="mb-6 flex items-center gap-3">
        <h1 className="text-lg font-semibold">Phantasi 磁贴预览</h1>
