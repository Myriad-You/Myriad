...
export function predictTextProsody(input: TextProsodyInput): SpeechProsodyPlan {
  ...
  for (const candidate of candidates.toSorted(
    ...
    // In particular, do not squeeze old accents along a new whole-reply character/duration ratio.
    ...
    // The speech source admits only a rolling window to the scheduler
  }
  return {
