...
export const zhCN: TranslationKeys = {
  ...
  config: {
    ...
    updaterHintUpdating:
      '系统正在自动完成更新，期间站点会短暂无法访问，完成后自动恢复。',
    updaterHintMaintenance:
      '系统处于维护状态。如果确认服务已恢复正常，可在下方「维护与恢复」中退出维护模式。',
    updaterHintNeedsManual:
      '自动回退没有完成。可以在下方「维护与恢复」中一键回到更新前的版本。',
    updaterHintOffline:
      '请检查 backend 是否配置了 MYRIAD_UPDATER_URL 和 UPDATE_TOKEN。',
    updaterHintFirstRun: '点「检查更新」即可获取当前通道的最新版本。',
    ...
    updaterDispatched: '更新已开始（任务 {jobId}），下方会实时显示进度。',
    updaterWhatHappensTitle: '更新时会发生什么？',
    updaterWhatHappens1: '先自动备份数据（创建快照）',
    updaterWhatHappens2: '进入维护模式，站点短暂无法访问（通常几分钟）',
    updaterWhatHappens3: '自动检查新版本是否正常；失败会自动回到原版本',
    updaterIrreversibleWarn:
      '本次更新包含不可逆的数据变更，更新后将无法直接回退到旧版本。',
    ...
    updaterChannelGroupDesc: '决定这台服务器跟随哪条线获取更新。点选即保存。',
    updaterChannelStable: '稳定版',
    updaterChannelStableDesc: '经过完整测试的正式版本，适合所有人。',
    updaterChannelPreview: '预览版',
    updaterChannelPreviewDesc: '抢先体验即将发布的新功能，可能有小问题。',
    updaterChannelDev: '开发版（跟随提交）',
    updaterChannelDevDesc:
      '跟随 preview 分支的每一个提交，最新也最不稳定，供开发者使用。',
    ...
    updaterConfirmUpgrade:
      '更新到 {version}？\n\n更新过程：\n1. 自动备份数据（创建快照）\n2. 进入维护模式，站点短暂无法访问\n3. 完成后自动检查，失败会自动回到原版本',
    updaterConfirmDowngrade:
      '⚠ 确定要降级吗？\n\n目标 {version} 比当前 {current} 更旧。降级后数据库可能无法自动回退，存在数据丢失或功能异常的风险。',
    updaterConfirmRisk:
      '⚠ 无法确认这次变更是否安全：目标版本与当前历史分叉、新旧关系未知，或包含不可逆的数据变更。\n\n继续可能损坏数据且无法回退。确定继续吗？',
    updaterConfirmRollback:
      '回到 {version} 更新前的状态？\n\n会用当时的快照恢复数据，快照之后产生的数据将丢失。',
    updaterConfirmExitMaintenance:
      '强制退出维护模式？\n\n只有在确认服务已恢复正常时才这样做，否则用户会访问到异常的站点。',
    updaterConfirmRescueContinue:
      '回到更新前的版本？\n\n将使用快照 {snapshotId} 恢复数据，并回到版本 {version}。',
    updaterMaintenanceGroup: '维护与恢复',
    updaterMaintenanceGroupDesc: '更新出问题时的处理入口，日常不需要使用。',
    updaterRescueContinue: '回到更新前的版本',
    updaterRescueContinueDesc:
      '用更新前自动创建的快照恢复数据并重启服务，是最安全的恢复方式。',
    updaterRescueContinueDispatched: '恢复已开始',
    updaterForceExit: '强制退出维护模式',
    updaterForceExitDesc:
      '只解除维护状态，不恢复数据。仅在确认服务已经正常时使用。',
    updaterMaintenanceExited: '已退出维护模式',
    updaterSelfUpdateNeeded:
      '新版本 {version} 需要先把更新器升级到 {minVersion} 以上，才能继续更新。',
    updaterSelfUpdateButton: '升级更新器',
    updaterSelfUpdateConfirm:
      '升级更新器到 {version}？\n\n升级只需几秒，站点服务不受影响。完成后再点「立即更新」继续。',
    updaterSelfUpdateDispatched:
      '更新器升级已开始，几秒后完成，之后可以继续更新。',
    ...
    updaterSnapshotGroupDesc:
      '每次更新前会自动备份数据。出问题时，可以从这里回到更新前的状态。',
    updaterNoSnapshots: '还没有备份。完成第一次更新后会自动出现。',
    updaterRollback: '回退到此版本',
    updaterRollbackDispatched: '回退已开始',
    updaterGroupAdvanced: '高级与诊断',
    updaterGroupAdvancedDesc: '技术细节与运维工具，日常使用无需打开。',
