...
pub fn mood_tone_instruction(mood: f64, arousal: f64) -> &'static str {
    match crate::affect::mood_band(mood, arousal) {
...
pub fn format_emotion_section(emotion: f64, emotion_arousal: f64) -> Option<String> {
    ...
    let valence = emotion - crate::affect::ORIGIN;
    let arousal = emotion_arousal - crate::affect::ORIGIN;
...
/// Her soul for speaking: who she is, the speaking contract, and her
/// personality. None when she has neither a name nor a personality.
pub fn format_persona(name: &str, personality: &str) -> Option<String> {
    let name = name.trim();
    let personality: String = personality.trim().chars().take(PERSONALITY_CHARS).collect();
...
mod tests {
    ...
    fn persona_contract_is_attached_even_on_name_only() {
        assert!(format_persona("", "").is_none());
        let named_text = format_persona("瞳", "").unwrap();
        assert!(named_text.starts_with("You are 瞳."));
        assert!(named_text.contains(PERSONA_SPEAKING_CONTRACT));
        let text = format_persona("瞳", "气质：认真").unwrap();
        ...
        let long = format!("{}末尾", "说".repeat(3000));
        assert!(format_persona("瞳", &long).unwrap().ends_with("末尾"));
